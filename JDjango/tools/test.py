import wx
import wx.adv
from wx.tools import wxget_docs_demo
...
class MemoDialog(wx.Dialog):
    ...
    """
    def __init__(self,parent=None,title="TTT",text="",pos=None,size=(500,500)):
...
class MainFrameGUI(wx.Frame):
    ...
    def __init__(self, parent = None):
        wx.Frame.__init__(self, parent, id = wx.ID_ANY, title = "wxpython所有示例及代码演示", pos = wx.DefaultPosition, size = wx.Size(900, 800), style = wx.DEFAULT_FRAME_STYLE|wx.TAB_TRAVERSAL)
        panel = TestPanel(self)
        # panel = wx.Panel(self)
        # panelSizer = wx.BoxSizer(wx.HORIZONTAL)
        # panel.SetSizer(panelSizer)
        # self.btn = buttons.GenButton(panel, -1, label='点击打开Demo程序')
        # panelSizer.Add(self.btn, 1, wx.EXPAND | wx.ALL, 2)
        # self.Bind(wx.EVT_BUTTON, self.onButtonClick, self.btn)

    def onButtonClick(self, e):
        # wx.tools.wxget_docs_demo.demo_main()
        dlg = MemoDialog(self)
        dlg.ShowModal()
        dlg.Close(True)
