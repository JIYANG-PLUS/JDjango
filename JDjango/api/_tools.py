...
def read_file_list_del_comment(path: str) -> List[str]:
    """删掉注释，读取文件列表（包含末尾的换行符\\n）
    
        注意：请不要使用除 # 之外的注释符，该限制将在未来被修复
    """
    patt = re.compile(r'(\s*#.*?$)')
...
def cut_content_by_doublecode(text: str, leftCode: str='[', rightCode: str=']', prefix='') -> str:
    """获取成对的中括号中的文本（不包含两侧成对符号）
    
        当提供前缀 prefix 时，将包含 双侧括号 输出
    """
    ...
    for _ in text:
        ...
        if leftCode == _:
            stack.append(_)
        if rightCode == _ and len(stack) > 0: # 右括号开始的情况排除
            stack.pop()
            if 0 == len(stack):
                if not prefix:
                    return cut_text[:-1]
                else:
                    return prefix + leftCode + cut_text[:-1] + rightCode
    return ''
